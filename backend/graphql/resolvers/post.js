...
module.exports = {
  //* GET_POSTS: if the user id isn't provided get all following posts, else if target account is public or logged user is a follower get posts
  Query: {
    async getPosts(parent, args, context, info) {
      ...
      const following = follows
        ...
        .push(username);
      if (!userID) {
        //* target user id not provided return all following posts
        const posts = following.map(
          async (currentUser) => await Post.find({ username: currentUser })
        );
        return (await Promise.all(posts)).flat().map((post) => ({
          ...post._doc,
          postID: post._id,
        }));
      } else {
        //* throw an error if target user doesn't exist
        const { username: targetUser, accountType } = await User.findById(
          userID
        );
        if (!username) {
          throw new Error("target user does not exist");
        }
        //* if logged user is blocked throw an error
        const isAuthUserBlocked = await Block.findOne({
          blocker: username,
          blocking: targetUser,
        });
        if (isAuthUserBlocked) {
          throw new Error("user is blocked");
        }
        if (accountType === "public" || following.indexOf(targetUser) > 0) {
          //* if target user's account is public or followed return his posts
          return await Post.find({ username: targetUser }).map((post) => ({
            ...post._doc,
            postID: post._id,
          }));
        } else {
          //* else throw an error
          throw new Error(
            "user account is private. follow this account first!"
          );
        }
      }
    },
    ...
    async getPost(parent, args, context, info) {
      ...
      //* if logged user is blocked throw an error
      const isAuthUserBlocked = await Block.find({
        blocker: post.username,
        blocking: username,
      });
      if (isAuthUserBlocked) {
        throw new Error("user is blocked");
  ...
  Mutation: {
    ...
    async updatePost(parent, args, context, info) {
      ...
      const res = await Post.updateOne(postID, {
    ...
    async react(parent, args, context, info) {
      ...
      const react = await React.find({ reactedBy: username, postID });
      if (react) {
        await react.remove();
        return {
          ...react._doc,
    ...
    async updateComment(parent, args, context, info) {
      ...
      const { username } = checkAuth(context);
      const { updateCommentInput: { commentID, newCommentBody } } = args;
